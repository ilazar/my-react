(() => {
  ...
  const createDOMElement = $parent => node => {
    ...
    if (typeof node === "string" || typeof node === "number") {
      ...
      if (node.length === 0) {
        $el = document.createTextNode("empty");
      } else {
        node
          .map(createDOMElement($parent))
          .forEach($parent.appendChild.bind($parent));
        // TODO find another solution, because if required to return a Node
      ...
      if (node.type.isClass) {
        ...
        return createDOMElement($parent)(virtualNode);
      } else if (
        node.type.typeof &&
  ...
  function updateDom($parent, newNode, oldNode, index = 0) {
    ...
    if (isNullOrUndefined(oldNode)) {
      $parent.appendChild(createDOMElement($parent)(newNode));
    } else if (isNullOrUndefined(newNode)) {
      $parent.removeChild($parent.childNodes[index]);
    } else if (changed(newNode, oldNode)) {
      $parent.replaceChild(
        createDOMElement($parent)(newNode),
  ...
  window.ReactDOM = {
    render: (rootReactEl, domEl) => {
      ...
      console.log("render:newRootReactElement", newRootReactElement);
      updateDom(domEl, newRootReactElement);
    },
    reRender: () => {
      console.log("reRender:newRootReactElement", newRootReactElement);
